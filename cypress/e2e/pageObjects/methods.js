...
export const methods = {
  ...
  openNewWindow(buttonLocator, expectedMessage) {
    ...
    cy.on('window:load', (newWin) => {
      cy.wrap(newWin).find('body').should('contain', expectedMessage);
      newWin.close();
    });
  },
  ...
  startAndStopProgressBar(stopValue) {
    ...
    locators.progressBarPage.progressBar().invoke('attr', 'aria-valuenow').then((value) => {
